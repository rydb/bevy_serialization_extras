...
impl From<GeometryWrapper> for Resolve<Mesh3dFlag, RequestAssetStructure<GltfPhysicsMeshPrimitive>> {
    fn from(value: GeometryWrapper) -> Self {
        match value.0 {
            urdf_rs::Geometry::Box { size } => {
                ...
                Resolve::One(Mesh3dFlag::Pure(MeshPrefab::Cuboid(Cuboid {
                    half_size: Vec3::new(
                        bevy_size[0] as f32,
                        bevy_size[1] as f32,
                        bevy_size[2] as f32,
            ...
            urdf_rs::Geometry::Cylinder { radius, length } => {
                ...
                Resolve::One(Mesh3dFlag::Pure(MeshPrefab::Cylinder(Cylinder {
            ...
            urdf_rs::Geometry::Capsule { radius, length } => {
                //TODO: double check that this is correct
                Resolve::One(Mesh3dFlag::Pure(MeshPrefab::Capsule(Capsule3d {
                    radius: radius as f32,
                    half_length: length as f32,
                }).into()))
            }
            urdf_rs::Geometry::Sphere { radius } => Resolve::One(Mesh3dFlag::Pure(MeshPrefab::Sphere(Sphere {
                radius: radius as f32,
            }).into())),
            urdf_rs::Geometry::Mesh { filename, .. } => {
                Resolve::Other(
                    RequestAssetStructure::<GltfPhysicsMeshPrimitive>::Path(filename)
                )
